...
在国内直播 App 无需 OBS 或 RTMP 推流入口的情况下，用手机作为最后一跳，将电脑端推送的画面直接喂给 App 的直播。

首选目标: 抖音、快手、小红书、微信等。

核心原则:
1. 画面安排由电脑端处理：分辨率、旋转角度、比例适配均在电脑端完成，手机端不做多余变换。
2. 手机端零自定义渲染：采用系统 MediaCodec 硬解直出至相机预览 Surface，实现零拷贝、原生画质与零额外渲染延迟。
3. 麦克风原生拾音：音频完全交给手机物理麦克风直接拾音，音质自然清晰。
4. 极低延迟与稳定传输：采用 SRT 传输，推流即开播，连接自动稳定维持。
5. 动态旁路开关：提供 START 与 STOP 总控开关，STOP 时放行系统原生相机，扫码与直播无缝切换。
